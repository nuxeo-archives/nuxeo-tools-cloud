...
# Prepare instance
gcloud compute instances create nuxeo-template-${tstamp} --image ubuntu-16-04 --machine-type ${TYPE} --zone ${ZONE} --metadata-from-file startup-script=$(dirname $(pwd))/common/bootstrap.sh --tags http-server,https-server
...
# Create image
gcloud compute images create nuxeo-lts-2016-${tstamp} --source-disk nuxeo-template-${tstamp} --source-disk-zone ${ZONE}
